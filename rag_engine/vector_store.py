...
def get_embedding_function(provider: str = "huggingface", api_key: Optional[str] = None, model_name: Optional[str] = None) -> Embeddings:
    ...
    if provider == "gemini":
        ...
        if not key: raise ValueError("Google API Key is required for Gemini embeddings.")
        ...
        embed_fn = HuggingFaceEmbeddings(model_name=model_name or "sentence-transformers/all-MiniLM-L6-v2", model_kwargs={"device": "cpu"}, encode_kwargs={"normalize_embeddings": True, "batch_size": 32})

    _EMBEDDING_CACHE[cache_key] = embed_fn
    return embed_fn


def build_vector_store(chunks: List[Document], embedding_function: Embeddings, persist_directory: Optional[str] = None, collection_name: str = "rag_documents"):
    """Fast vector indexing with C++ HNSW graph in ChromaDB or In-Memory fallback."""
    if not chunks:
    ...
    try:
        ...
        return Chroma.from_documents(documents=chunks, embedding=embedding_function, persist_directory=persist_directory, collection_name=collection_name)
    except Exception:
...
def load_persisted_vector_store(embedding_function: Embeddings, persist_directory: str = "./chroma_db", collection_name: str = "rag_documents"):
    """Load persisted ChromaDB from disk if available."""
