...
def render_token_card(ci: dict):
    ...
    web = " | 🌐 Web Augmented" if ci.get('web_search_used') else ""
    st.markdown(f'<div class="token-card"><span>🪙 {ci.get("total_tokens", 0)} Toks ({ci.get("input_tokens", 0)} in / {ci.get("output_tokens", 0)} out)</span><span>💰 {cost}</span><span>🛡️ {ci.get("groundedness_score", 100)}% Grounded{savings}{web}</span></div>', unsafe_allow_html=True)

def render_sources(sources: list):
    if not sources: return
    with st.expander(f"📖 View Referenced Sources ({len(sources)} chunks)", expanded=False):
...
with st.sidebar:
    ...
    env_groq, env_gemini = os.getenv("GROQ_API_KEY", "").strip(), os.getenv("GOOGLE_API_KEY", "").strip()

    if provider == "groq":
        u_key = st.text_input("⚡ Groq API Key", type="password", placeholder="✅ Configured via Secrets" if env_groq else "Enter Groq API Key (gsk_...)")
        api_key = u_key.strip() or env_groq
        model_name = st.selectbox("Model", ["openai/gpt-oss-120b", "openai/gpt-oss-20b", "qwen/qwen3.6-27b", "groq/compound-mini", "llama-3.3-70b-versatile", "llama-3.1-8b-instant"])
    elif provider == "gemini":
        u_key = st.text_input("🔑 Gemini API Key", type="password", placeholder="✅ Configured via Secrets" if env_gemini else "Enter Gemini API Key (AIzaSy...)")
        api_key = u_key.strip() or env_gemini
        ...
        if st.session_state.files_hash != cur_hash:
            with st.spinner(f"Indexing {len(uploaded_files)} document(s)..."):
                try:
                    ...
                    embed_fn = get_embedding_function(embed_provider, api_key if embed_provider == "gemini" else None)
                    st.session_state.vector_store = build_vector_store(all_chunks, embed_fn, persist_directory="./chroma_db")
    ...
    # Library Metrics
    if m := st.session_state.doc_metadata:
        st.markdown("---")
        c1, c2, c3 = st.columns(3)
        c1.metric("Docs", m['total_books']); c2.metric("Pages", m['total_pages']); c3.metric("Chunks", m['total_chunks'])
...
if not st.session_state.vector_store:
        ...
        if st.button("🧪 Run Automated RAG Triad Benchmark", use_container_width=True):
            with st.spinner("Auditing RAG pipeline against synthetic test cases..."):
                try:
                    ...
                    scores = [(t["question"], evaluate_rag_triad(t["question"], eval_llm.invoke(f"Context:\n{t['question']}").content, retriever.invoke(t["question"]), eval_llm)) for t in test_cases]
                    st.metric("🏆 Overall RAG Quality Score", f"{round(sum(s[1]['overall_score'] for s in scores) / len(scores), 1)}%")
                    st.dataframe([{"Question": s[0], "Context Rel.": f"{s[1]['context_relevance']['score']}%", "Faithfulness": f"{s[1]['faithfulness']['score']}%", "Answer Rel.": f"{s[1]['answer_relevance']['score']}%", "Composite": f"{s[1]['overall_score']}%", "Status": s[1]['status']} for s in scores], use_container_width=True)
                except Exception as e:
    ...
    for msg in st.session_state.chat_history:
        with st.chat_message(msg["role"]):
            ...
            render_token_card(msg.get("crag_info"))
            render_sources(msg.get("sources"))

    # Query Input
    if user_query := st.chat_input("Ask any question across your uploaded documents..."):
            ...
            with st.chat_message("assistant"):
                ...
                if enable_crag:
                    with st.spinner("⚡ Executing LangGraph CRAG & Token Pruning..."):
                        stream, source_docs, crag_stats = stream_langgraph_crag_pipeline(user_query, retriever, hist, provider, model_name, temp, api_key, base_url, enable_web_search=enable_web)
                    answer_text = st.write_stream(stream)
                    render_token_card(crag_stats)
                else:
                    with st.spinner("Generating grounded answer..."):
                        res = create_rag_chain(retriever, get_llm(provider, model_name, temp, api_key, base_url)).invoke({"question": user_query, "chat_history": hist})
